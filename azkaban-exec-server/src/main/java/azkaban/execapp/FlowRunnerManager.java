  ...
  public void submitFlow(final int execId) throws ExecutorManagerException {
    ...
    Map<String, String> flowParameters = options.getFlowParameters();
      for (Map.Entry<String, String> entry : flowParameters.entrySet()) {
        ...
        if (!"".equals(value)) {
//        logger.info("modify parameters：key=>" + entry.getKey() + ",value=>" + value);
          logger.info("receive parameters：key=>" + entry.getKey() + ",value=>" + value);
          if (value.startsWith("[") && value.endsWith("]")) {
            ...
            switch (timeFormat.length()) {
              ...
              case 12:
                ...
                break;
              case 14:
                rightNow.add(Calendar.SECOND, offset);
                break;
              default:
                rightNow.add(Calendar.DAY_OF_YEAR, offset);
            }
            String paramDate = formatt.format(rightNow.getTime());
            flowParameters.put(entry.getKey(), paramDate);

          }
        }
    }

    options.getFlowParameters().forEach((k, v) -> {
      logger.info("modifyed execution options key is " + k + ",value is " + v);
    });
